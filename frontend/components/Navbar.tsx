...
const Navbar: React.FC = () => {
    ...
    <div className="group relative h-full flex items-center">
      ...
      {children && (
        <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2 opacity-0 -translate-y-5 pointer-events-none transition-all duration-300 ease-in-out group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto">
          <div
            className={`w-48 rounded-xl border border-white/10 overflow-hidden shadow-2xl ${
              scrolled ? 'bg-black/95' : 'bg-blue-600'
            }`}
          >
  ...
  return (
    <>
      ...
      ====================== */}
      <nav
        className={`fixed top-0 left-1/2 -translate-x-1/2 z-50 transition-[max-width,background-color,border-radius,padding] duration-500 linear hidden lg:flex items-center justify-between w-full ${
          scrolled
      ...
      <div className="lg:hidden fixed bottom-4 left-4 right-4 z-50">
        <div className="bg-zinc-900/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl flex items-center justify-around px-6 py-4">
          ...
          <Link
            to="/feed"
            className={`flex flex-col items-center gap-1 ${
              isActive('/feed') ? 'text-blue-500' : 'text-gray-500'
            }`}
          >
            <Radio size={20} />
            {isActive('/feed') && (
          ...
          <Link
            to="/rankings"
            className={`flex flex-col items-center gap-1 ${
              isActive('/rankings') ? 'text-blue-500' : 'text-gray-500'
            }`}
          >
            <Trophy size={20} />
            {isActive('/rankings') && (
